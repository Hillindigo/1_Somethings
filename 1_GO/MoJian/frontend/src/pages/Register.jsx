...
// Register 注册页面，集成图片验证码
export default function Register() {
  const { register } = useAuth()
  const navigate = useNavigate()
  const [form, setForm] = useState({ username: '', password: '', email: '', captchaCode: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    setError('')
    try {
      await register(form.username, form.password, form.email, captchaId, form.captchaCode)
      navigate('/login')
    } catch (err) {
      // 注册失败后刷新验证码
      fetchCaptcha()
    ...
    <div className="max-w-md mx-auto px-6 pt-30">
      <div className="article-enter">
        <h1 className="font-display text-5xl font-black mb-2">
          注册<span className="text-accent">.</span>
        </h1>
        <p className="text-muted mb-10">开始你的写作之旅</p>
        ...
        <p className="text-center text-sm text-muted mt-8">
          已有账号？{' '}
          <Link to="/login" className="text-accent hover:underline">去登录</Link>
        </p>
      </div>
    </div>
  )

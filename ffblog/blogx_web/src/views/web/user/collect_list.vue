...
async function dispatchDelete(idList: number[]) {
  const res = await collectArticleRemoveApi({
    articleIDList: idList,
    collectID: Number(route.query.collectID)
  })
  if (res.code) {
...
<template>
  <div class="user_article_list_view">
    ...
    <f_article_list ref="fArticleListRef" @dispatch-delete="dispatchDelete" :is-check="baseStore.isMe && route.query.collectID > 0"></f_article_list>
  </div>
</template>

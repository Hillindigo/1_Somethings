...
import type { commentCreateRequest,  commentListType} from "@/api/comment_api";
  ...
  <div class="article_comment_view">
    <comment_list ref="commentListRef" :type="1" v-slot="{item}:{item: commentListType}">
      ...
      <div class="info">
        <div class="nickname">
          ...
          <span class="article" v-if="!item.articleCover">
              评论了文章： <router-link :to="{name: 'articleDetail', params: {id: item.articleID}}">{{ item.articleTitle }}</router-link>
            </span>
        </div>
        <div class="content">
      ...
      <div class="cover" v-if="item.articleCover">
        <img @click="goArticle(item.articleID)" :src="item.articleCover" alt="">
        <span>
            <a-typography-text @click="goArticle(item.articleID)"  :ellipsis="{rows: 1, css: true}">文章：{{ item.articleTitle }}</a-typography-text>
          </span>
      </div>
    </comment_list>
  </div>
